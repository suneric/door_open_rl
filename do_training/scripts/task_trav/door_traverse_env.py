...
class DoorTraverseTaskEnv(DoorOpenTaskEnv):
    ...
    def _load_door_pull_agent(self,pull_policy,pull_model):
        if pull_policy == 'ppo':
            ...
            model_folder = "ppo_noise"+str(self.noise)
            actor_path = os.path.join(sys.path[0], '..', "policy", "door_pull", model_folder, "logits_net", pull_model)
            critic_path = os.path.join(sys.path[0], '..', "policy", "door_pull", model_folder, "val_net", pull_model)
            ...
            model_path = os.path.join(sys.path[0], '..', 'policy', 'door_pull', "dqn_noise"+str(self.noise), pull_model)
